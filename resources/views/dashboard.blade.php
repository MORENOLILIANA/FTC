        ...
        <section>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="card bg-gray-900 border border-gray-800 rounded-xl p-5">
                    ...
                    <div class="text-3xl font-bold text-white">{{ number_format($stat['value']) }}</div>
        ...
        <section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="card bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 flex items-center gap-4">
                    <div class="w-9 h-9 rounded-full flex items-center justify-center {{ $check['ok'] ? 'bg-emerald-500/10' : 'bg-red-500/10' }}">
                        <span class="text-lg">{{ $check['ok'] ? '✅' : '❌' }}</span>
                    </div>
                    <div>
                        <div class="font-medium text-sm">{{ $check['label'] }}</div>
                        <div class="text-xs text-gray-400">{{ $check['detail'] }}</div>
                    </div>
                </div>
                @endforeach
            </div>
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                    ...
                    <div class="divide-y divide-gray-800/50">
                        ...
                        <div class="px-5 py-3 flex items-center gap-4 hover:bg-gray-800/30 transition-colors">
                            <span class="text-xs font-mono font-bold px-2 py-0.5 rounded border {{ $methodColors[$route[0]] ?? '' }} min-w-[52px] text-center">
                                {{ $route[0] }}
                            </span>
                            <code class="text-sm text-gray-300 flex-1">{{ $route[1] }}</code>
                            <span class="text-xs text-gray-500 hidden md:block">{{ $route[2] }}</span>
                        </div>
                        @endforeach
                    </div>
                </div>
